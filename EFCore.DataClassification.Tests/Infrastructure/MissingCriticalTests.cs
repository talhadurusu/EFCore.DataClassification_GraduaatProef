...
public class MissingCriticalTests {
    ...
    public void BugCatcher_MixedOperations_AllMustHaveCorrectOrder() {
        ...
        if (emailRemove != null && emailRename != null && emailCreate != null) {
            ...
            var createIdx = operations.IndexOf(emailCreate);
            
            Assert.True(removeIdx < renameIdx,
                $"BUG: Email Remove ({removeIdx}) must come before Rename ({renameIdx})");
            Assert.True(renameIdx < createIdx,
                $"BUG: Email Rename ({renameIdx}) must come before Create ({createIdx})");
        }

        if (phoneRemove != null && phoneAlter != null && phoneCreate != null) {
            var removeIdx = operations.IndexOf(phoneRemove);
            var alterIdx = operations.IndexOf(phoneAlter);
            var createIdx = operations.IndexOf(phoneCreate);
            
            Assert.True(removeIdx < alterIdx,
                $"BUG: Phone Remove ({removeIdx}) must come before Alter ({alterIdx})");
            Assert.True(alterIdx < createIdx,
                $"BUG: Phone Alter ({alterIdx}) must come before Create ({createIdx})");
        }

        if (ssnRemove != null && ssnDrop != null) {
            Assert.True(operations.IndexOf(ssnRemove) < operations.IndexOf(ssnDrop),
                $"BUG: SSN Remove must come before Drop");
        }

        if (addressAdd != null && addressCreate != null) {
            Assert.True(operations.IndexOf(addressAdd) < operations.IndexOf(addressCreate),
                $"BUG: Address Add must come before Create classification");
        }
    }
